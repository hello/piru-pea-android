...
/**
 * The log contract is used to write and read the log entries into the database. Do not use directly when writing, use {@link no.nordicsemi.android.log.Logger} member methods instead.
 */
public class LogContract {
	...
	protected interface LogColumns {
		...
		/** Log entry level. See {@link no.nordicsemi.android.log.LogContract.Log.Level} for options. */
	...
	public final static class Log implements BaseColumns, LogColumns {
		...
		/**
		 * Returns the {@link android.net.Uri} to log entries from session with given id.
		 * 
		 * @param sessionId
		 *            the session id
		 * @return the {@link android.net.Uri} that can be used to read log entries from
		 */
		...
		/**
		 * The MIME type of {@link #CONTENT_URI} providing a directory of log entries.
		 */
		public static final String CONTENT_TYPE = "vnd.android.cursor.dir/no.nordicsemi.android.log.enties";

		/**
		 * The MIME type of a {@link #CONTENT_URI} subdirectory of a single log entry.
		 */
	...
	public final static class Session implements BaseColumns, SessionColumns {
		...
		/**
		 * Returns the {@link android.net.Uri} to session with given id.
		 * 
		 * @param sessionId
		 *            the session id
		 * @return the {@link android.net.Uri} that can be used to read session log entries
		 */
		public static Uri createUri(final long sessionId) {
		...
		/**
		 * Returns the {@link android.net.Uri} to session with given key and number.
		 * 
		 * @param key
		 *            the session key
		 * @param number
		 *            the session number. Numbers are incremented automatically for each key starting from 1 when creating new session.
		 * @return the {@link android.net.Uri} that can be used to read session
		 */
		public static Uri createUri(final String key, final int number) {
		...
		/**
		 * Returns the {@link android.net.Uri} to sessions from given application.
		 * 
		 * @param applicationId
		 *            the application id
		 * @return the {@link android.net.Uri} that can be used to read log sessions
		 */
	...
	public final static class Application implements BaseColumns, ApplicationColumns {
		 ...
		 */
		public static final Uri CONTENT_URI = Uri.withAppendedPath(AUTHORITY_URI, "application");
